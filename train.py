...
from data_loading import (load_preprocessed_data, 
...
def train_model(
    ...
    # data_path = './data/'
    # data_file = glob.glob(os.path.join(data_path, '*'))[0]
    data_file = "./data/train_eye_dataset.h5"

    # 1. 加载数据    dict_keys(['images', 'masks', 'labels', 'filenames'])
    dataset = load_preprocessed_data(data_file)
    ...
    # 创建训练集和验证集字典
    train_dataset = {key: [dataset[key][i] for i in train_indices] for key in dataset.keys()}
    val_dataset = {key: [dataset[key][i] for i in val_indices] for key in dataset.keys()}
    ...
    epoch = 1  # Initialize epoch counter manually
    best_dice_score = 0
    ...
    while True:
        ...
        with tqdm(total=steps, desc=f'Epoch {epoch}', unit='step') as pbar:
            for step in range(steps):
                ...
                random_indices = np.random.randint(0, len(filtered_sample_map[0]), batch_size)
                ...
                for i in range(batch_size):
                    ...
                    label_patch = labels_data_pool[img_idx, :, x_start:x_end, y_start:y_end]
                ...
                batch_images = batch_images.to(device=device, dtype=torch.float32, memory_format=torch.channels_last)
                ...
                with torch.autocast(device.type if device.type != 'mps' else 'cpu'):
                    ...
                    loss += dice_loss(torch.sigmoid(masks_pred.squeeze(1)), batch_labels.squeeze(1), multiclass=False)
        ...
        batch_images_val = batch_images_val.to(device=device, dtype=torch.float32, memory_format=torch.channels_last)
        ...
        with torch.no_grad():
            ...
            dice_score = dice_coeff(masks_pred_binary, batch_labels_val, reduce_batch_first=False)
        
            # 计算每个类别的Dice分数
            dice_score_bg = dice_coeff(masks_pred_binary, batch_labels_val, reduce_batch_first=False)
            masks_pred_binary = (torch.sigmoid(masks_pred) <= 0.5).float()
            dice_score_fg = dice_coeff(masks_pred_binary, 1 - batch_labels_val, reduce_batch_first=False)
        ...
        print(f"Epoch {epoch} - Average loss: {epoch_loss/steps:.4f} - Dice score: {dice_score:.4f} - Avg Dice score: {dice_score_avg:.4f} - Best dice score: {best_dice_score:.4f}")
        ...
        concat_image = np.concatenate((sample_image, blank_image, sample_pred, blank_image, sample_label), axis=1)

        concat_image = np.array(concat_image * 255).astype(np.uint8).transpose(1, 2, 0)

        Image.fromarray(concat_image).save(f"visualizations/{epoch:03d}_{sample_num:03d}.png")
...
def get_args():
    ...
    parser.add_argument('--data-file', '-d', type=str, default='./data/train_eye_dataset.h5',
    ...
    parser.add_argument('--patch-size', '-p', dest='patch_size', type=int, default=128,
    ...
    parser.add_argument('--early-stopping-patience', '-esp', dest='early_stopping_patience', type=int, default=20,

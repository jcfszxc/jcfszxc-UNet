...
def eval_model(
    ...
    batch_images_test = batch_images_test.to(device=device, dtype=torch.float32, memory_format=torch.channels_last)
    ...
    with torch.no_grad():
        for start_idx in range(0, total_samples, inference_batch_size):
            ...
            mini_batch_images = mini_batch_images.to(device=device, dtype=torch.float32, memory_format=torch.channels_last)
    ...
    for i in range(n_test):
        ...
        concat_image = np.concatenate((image_img, blank_image, pred_img, blank_image, label_img), axis=1)
        ...
        dice_score = dice_coeff(torch.from_numpy(masks_pred_binary), 
...
def get_args():
    ...
    parser.add_argument('--patch-size', '-p', type=int, default=128,

...
def preprocess_dataset(save_method="pickle", include_test=True):
    ...
    # 保存训练集
    train_output_file = save_data(
        train_dataset, output_dir, "train_eye_dataset", save_method
    )
    train_info = {
        "num_samples": len(train_dataset["images"]),
        "image_shape": (
            train_dataset["images"].shape if len(train_dataset["images"]) > 0 else None
        ),
        "mask_shape": (
            train_dataset["masks"].shape if len(train_dataset["masks"]) > 0 else None
        ),
        "label_shape": (
            train_dataset["labels"].shape if len(train_dataset["labels"]) > 0 else None
        ),
    ...
    if include_test:
        ...
        # 保存测试集
        test_output_file = save_data(
            test_dataset, output_dir, "test_eye_dataset", save_method
        )
        test_info = {
            ...
            "mask_shape": (
                test_dataset["masks"].shape if len(test_dataset["masks"]) > 0 else None
            ),
            "label_shape": (
                test_dataset["labels"].shape
                if len(test_dataset["labels"]) > 0
                else None
            ),
...
def process_data_subset(data_path, subset_name="数据集"):
    ...
    for image_path in glob.glob(images_path + "*.tif"):
        ...
        print(
            f"处理{subset_name} {image_name}: 图像形状: {image_numpy.shape}, 掩码形状: {mask_numpy.shape}, 标签形状: {label_numpy.shape}"
        )
